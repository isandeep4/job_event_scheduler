...
describe("EventProcessor", () => {
    ...
    describe("with no events in Redis", () => {
        it("should process 0 events", async() => {
            ...
            //Act
            const eventProcessor = new EventProcessor(redis, bigQuery, eventsNamespace);
    ...
    describe("with 1 event in Redis", () => {
        it("should process 1 event", async() => {
            ...
            const redis = mock<IRedis>();
            const bigQuery = mock<IBigQuery>();
            const eventString = JSON.stringify({eventName: "test", eventData: { testKey1: 123, testKey2: 'value' }});
            redis.lPopOne.mockResolvedValue(enrichEvent(eventString, ['lilysgarden']));
